<?php
/**
 * Front-end functionality for ZuidWest Staart plugin.
 *
...
function zw_staart_fetch_top_posts() {
	return (array) get_option( 'zw_staart_top_posts', array() );
}
...
function zw_staart_podcast_promo_block() {
	...
	if ( empty( $description ) || empty( $artwork_url ) || empty( $spotify_url ) || empty( $apple_url ) ) {
		return '';
	}
		...
		<div style="display: flex; flex-direction: column; gap: 12px; margin-top: 12px;">
			...
			<div>
				...
				<div style="display: flex; gap: 8px; flex-wrap: wrap;">
					<a href="<?php echo esc_url( $spotify_url ); ?>" target="_blank" rel="noopener noreferrer" class="plausible-event-name=Recirculatie+Podcast+Spotify" style="display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; background-color: #1DB954; color: white; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 0.85em;">
					...
					<a href="<?php echo esc_url( $apple_url ); ?>" target="_blank" rel="noopener noreferrer" class="plausible-event-name=Recirculatie+Podcast+Apple" style="display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; background-color: #A45EE5; color: white; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 0.85em;">
...
function zw_staart_top_posts_list() {
	...
	$post_id_to_url_mapping = array();
	foreach ( $output_posts as $p ) {
		$post_id = $p['post_id'] ?? 0;
		if ( $post_id ) {
			$post_id_to_url_mapping[ $post_id ] = get_permalink( $post_id );
		}
	}
		...
		<ol style="margin: 0; padding-left: 20px;">
			<?php foreach ( $output_posts as $p ) : ?>
				<?php
					$post_id = $p['post_id'] ?? 0;
				if ( ! $post_id ) {
					continue; // Skip if post ID wasn't resolved.
				}
				...
				<li class="zw-staart-top-post-item" data-post-id="<?php echo esc_attr( $post_id ); ?>" style="margin-bottom: 10px;">
					<a href="<?php echo esc_url( $post_permalink . '?utm_source=recirculatie' ); ?>" class="plausible-event-name=Recirculatie+Artikel" style="text-decoration: none;">
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			var postIdToUrlMapping = <?php echo wp_json_encode( $post_id_to_url_mapping, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT ); ?>;
			var minPostsDisplay = <?php echo intval( ZW_STAART_MIN_POSTS_DISPLAY ); ?>;

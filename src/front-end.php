...
function zw_staart_podcast_promo_block()
{
    ...
    ob_start();
    ?>
    <aside id="zw-staart-podcast-promo" style="margin-top: 20px; border: 1px solid #e0e0e0; border-radius: 8px; padding: 24px; background-color: #fafafa;">
        ...
        <div style="display: flex; gap: 20px; align-items: flex-start;">
            <?php if (!empty($artwork_url)): ?>
                <img src="<?php echo esc_url($artwork_url); ?>" alt="<?php echo esc_attr($title); ?>" style="width: 140px; height: 140px; border-radius: 6px; object-fit: cover; flex-shrink: 0;" />
            <?php endif; ?>
            <div style="flex: 1;">
                <h4 style="margin: 0 0 8px 0; font-size: 1.15em; font-weight: 600;"><?php echo esc_html($title); ?></h4>
                <p style="margin: 0 0 16px 0; line-height: 1.5; color: #666;"><?php echo esc_html($description); ?></p>
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <?php if (!empty($spotify_url)): ?>
                        <a href="<?php echo esc_url($spotify_url . '?utm_source=recirculatie'); ?>" class="plausible-event-name=Podcast+Spotify" style="display: inline-block; padding: 10px 18px; background-color: #1DB954; color: white; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 0.9em;">
                            Luister op Spotify
                        </a>
                    <?php endif; ?>
                    <?php if (!empty($apple_url)): ?>
                        <a href="<?php echo esc_url($apple_url . '?utm_source=recirculatie'); ?>" class="plausible-event-name=Podcast+Apple" style="display: inline-block; padding: 10px 18px; background-color: #A45EE5; color: white; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 0.9em;">

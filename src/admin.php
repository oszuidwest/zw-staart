<?php
/**
 * Admin functionality for ZuidWest Staart plugin.
 *
...
function zw_staart_migrate_from_old_version() {
	...
	$new_settings = get_option( 'zw_staart_settings', array() );
...
function zw_staart_settings_page() {
	...
	$plausible_days     = zw_staart_get_setting( 'plausible', 'days' ) ? zw_staart_get_setting( 'plausible', 'days' ) : ZW_STAART_DEFAULT_DAYS;
...
function zw_staart_get_top_posts() {
	$api_key      = zw_staart_get_setting( 'plausible', 'api_key' );
	$site_id      = zw_staart_get_setting( 'plausible', 'site_id' );
	$api_endpoint = zw_staart_get_setting( 'plausible', 'endpoint' );
	$days         = intval( zw_staart_get_setting( 'plausible', 'days' ) ? zw_staart_get_setting( 'plausible', 'days' ) : ZW_STAART_DEFAULT_DAYS );

	// Validate required settings.
	if ( empty( $api_key ) || empty( $site_id ) || empty( $api_endpoint ) ) {
		return;
	}

	// Calculate date range using wp_date() to respect WordPress timezone settings.
	$end_date   = wp_date( 'Y-m-d' ); // Today.
	$start_date = wp_date( 'Y-m-d', strtotime( '-' . ( $days - 1 ) . ' days' ) ); // X days ago (for a total of X days including today).
	...
	$request_data = array(
		...
		'order_by'   => array( array( 'pageviews', 'desc' ) ),
		'pagination' => array(
			'limit'  => ZW_STAART_API_PAGINATION_LIMIT,
			'offset' => 0,
		),
	);
	...
	usort( $filtered_articles, static fn( $a, $b ) => $b['pageviews'] <=> $a['pageviews'] );

	$top_articles = array_slice( $filtered_articles, 0, ZW_STAART_MAX_TOP_ARTICLES );

	// Resolve post IDs in batch to avoid N+1 queries later.
	$top_articles = zw_staart_resolve_post_ids( $top_articles );

	zw_staart_save_top_posts( $top_articles );
}
 ...
 *
 * @param array $top_posts_details Array of top post details to be saved.
 */
function zw_staart_save_top_posts( $top_posts_details ) {
	update_option( 'zw_staart_top_posts', $top_posts_details );
}

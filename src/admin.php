...
function zw_staart_get_top_posts()
{
    ...
    if (is_wp_error($response)) {
        error_log('ZuidWest Staart: API call failed - ' . $response->get_error_message());
        return;
    }

    $response_code = wp_remote_retrieve_response_code($response);
    if ($response_code != 200) {
        error_log('ZuidWest Staart: API returned non-200 status - ' . $response_code . ': ' . wp_remote_retrieve_response_message($response));
        return;
    }
